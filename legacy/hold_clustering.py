...
def dominant_color(image, detection, classified_holds_folder, image_file, index, k=2, plotting=False):
    ...
    if plotting:
        ...
        os.makedirs(classified_holds_folder + "/colours/", exist_ok=True)
        plt.savefig(classified_holds_folder + "/colours/" + image_file[:-4] + f'_{index}_color.png')
...
if __name__ == "__main__":
    # Change based on the image and detection file you want to use
    image_file = './test3.png'
    detection_file = './classified_images/test3_detection_results.txt'
    ...
    for index, detection in enumerate(detections):
        image = cv2.imread(image_file)
        colour = dominant_color(image, detection, classified_holds_folder, image_file, index, plotting=True)
